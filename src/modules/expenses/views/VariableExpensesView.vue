...
const uniquePayers = ref(
  Array.from(new Set(variableExpenses.value.map((expense) => expense.payer))),
)

const uniqueClassifications = ref(
  Array.from(new Set(variableExpenses.value.map((expense) => expense.classification))),
)
</script>
          ...
          <tbody>
            <tr v-for="classification in uniqueClassifications" class="hover:bg-base-300">
              ...
              <td>
                {{
                  variableExpenses
                    .filter((expense) => expense.classification === classification)
                    .map((expense) => expense.amount)
                    .reduce((acc, amount) => acc + amount, 0)
                }}
                €
