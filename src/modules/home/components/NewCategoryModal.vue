...
const emits = defineEmits(['createCategory', 'cancel'])
...
const onCreateCategory = () => {
  ...
  emits('createCategory', categoryName.value)
  categoryName.value = ''
}
...
<template>
  ...
  >
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
      ...
      <label class="text-md text-gray-700 mb-2">Nombre de la categoría:</label>
      <input
        type="text"
        class="input h-8 mt-3 w-full"
        v-model="categoryName"
        @keypress.enter="onCreateCategory"
      />
