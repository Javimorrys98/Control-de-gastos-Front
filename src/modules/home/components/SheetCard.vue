...
<template>
  ...
  >
    <RouterLink :to="`/${sheet._id}`" class="h-10 w-15"></RouterLink>
    <RouterLink
      :to="`/${sheet._id}`"
      class="text-xl font-bold text-white w-full text-center overflow-ellipsis"
    >
      {{ sheet.name }}
    </RouterLink>
    <div
      class="h-15 w-15 bg-blue-600 flex justify-center items-center opacity-0 transition-opacity group-hover:opacity-100 hover:bg-blue-500"
